 ...
 */
public class MaxHeapImpl<T> extends HeapOperationsImpl<T>{
    private int position=0;
    ArrayList<Node<T>> heapArr = new ArrayList<Node<T>>();
    final private Comparator<T> comparator;

    public MaxHeapImpl(Comparator<T> comparator) {
    ...
    public void bubbleDown(Node<T> root) {
        ...
        if (right<=heapArr.size() && comparator.compare((T)heapArr.get(right).getElement(),(T) heapArr.get(largest).getElement() ) >=0 )
            largest= right;
        if (largest!= index) {
            exchange( index, largest );
            bubbleDown( heapArr.get(largest) );
        }
    }
